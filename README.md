...
Facet-счётчики используют disjunctive (exclude-self) подход:

- **Hobby facets** считаются с учётом `q` и `nationalities`, но **без** текущего hobby-фильтра. Счётчик показывает, сколько пользователей (из числа подходящих под `q` и nationalities) имеют это хобби — то есть «что будет в результате, если добавить это хобби к уже выбранным».
- **Nationality facets** считаются с учётом `q` и `hobbies`, но **без** текущего nationality-фильтра. Счётчик показывает, сколько пользователей этой национальности соответствуют остальным фильтрам — то есть «сколько новых результатов появится, если добавить эту национальность».

Это делает sidebar полезным навигатором: пользователь видит реальные числа для каждого возможного следующего шага, а не уже применённые ограничения.
